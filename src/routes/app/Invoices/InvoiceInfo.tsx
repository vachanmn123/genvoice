...
export default function InvoiceInfo() {
  ...
  const [lineItems, setLineItems] = useState<LineItem[]>(
    invoice.items.map((item) => ({
      ...
      productId: products.find((p) => p.name === item.description)?.id || "",
        ...
        <div className="w-full sticky top-6 self-start">
          <Card>
            ...
            <CardContent className="overflow-y-scroll max-h-[80vh]">
              <Link to={`/app/invoices/${id}/print`} target="_blank">
                <Button className="w-full mb-5">Print</Button>
              </Link>
